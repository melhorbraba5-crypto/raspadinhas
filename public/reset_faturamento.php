...
try {
    // Prepara a query para zerar o faturamento atual do jogo especificado
    $stmt = $pdo->prepare("UPDATE bonus_system SET current_faturamento = 0.00 WHERE game_name = ?");
    ...
    if ($stmt->rowCount() > 0) {
        $_SESSION['form_feedback'] = [
            'type' => 'success',
            'message' => 'Faturamento do jogo ' . htmlspecialchars($game_name) . ' foi zerado com sucesso!'
        ];
    } else {
        $_SESSION['form_feedback'] = [
            'type' => 'error',
    ...
    error_log("Erro ao zerar faturamento: " . $e->getMessage());
    $_SESSION['form_feedback'] = [
        'type' => 'error',
        'message' => 'Erro de banco de dados ao tentar zerar o faturamento.'
    ];
}
